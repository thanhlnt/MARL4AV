...
to setup
  ...
  set damaged-inlane-duration-max 3600 ; 1800 ~ 30 minutes
  set damaged-inrescue-duration-max 3600 ; 2700 ~ 45 minutes
  ...
  ifelse (move-strategy = "Greedy") [
    ...
    if (move-strategy = "SARSA") or
       (move-strategy = "Q-Learning") or
       (move-strategy = "Double Q-Learning") [
      setup-tabular-algos
    ]

    if (move-strategy = "Deep Q-Learning") or
       (move-strategy = "Deep Q-Network") or
       (move-strategy = "Double Deep Q-Network") [
      setup-approximate-algos
    ]

    if (move-strategy = "Proximal Policy Optimization")  [
      setup-ppo
    ]
  ]
...
to go
  ...
  if ( (move-strategy = "Greedy") or (move-strategy = "Greedy-CL") ) [
    ;ask cars with [speed > 0] [ move-forward-greedy ]
    ask cars [ move-forward-greedy ]
  ]

  if (move-strategy = "SARSA") or
     (move-strategy = "Q-Learning") [
    go-sarsa-qlearning
  ]

  if (move-strategy = "Double Q-Learning") [
    go-double-ql
  ]
...
to setup-tabular-algos
  ...
  if (move-strategy = "SARSA") or (move-strategy = "Q-Learning") [
...
to setup-approximate-algos
  ...
  if (move-strategy = "Deep Q-Network") or
     (move-strategy = "Double Deep Q-Network") [
...
to setup-ppo
  ...
  if move-strategy = "Soft Actor-Critic" [
    py:set "memory_size" memory-size
    py:set "batch_size" batch-size
...
to go-approximate-algos
  ...
  if move-strategy = "Double Deep Q-Network"  [
    (py:run
...
to move-forward-rl ; car procedure
  ifelse (prob-damaged-car <= random-float 1) [
    ...
    set reward (log (speed + 1e-8) 2) + (log ((patience - patience-current) / patience + 1e-8) 2) ; reward can be positive or negative
...
to slow-down-car ; car procedure
  if speed > deceleration [
    ...
    set patience-current patience-current - 1 ; every time you hit the brakes, you loose a little patience
...
move-strategy
move-strategy
"Greedy" "Q-Learning" "SARSA" "Double Q-Learning" "Deep Q-Learning" "Deep Q-Network" "Double Deep Q-Learning" "Proximal Policy Optimization"
7
...
"" ""
PENS
"reward" 1.0 0 -2674135 true "" "plot sum [reward] of cars with [speed > 0]"
